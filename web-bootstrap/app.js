...
function initTheme() {
    ...
    // Bootstrap class for dark/light mode
...
function setupEventListeners() {
    ...
    if (themeToggle) {
        themeToggle.addEventListener('click', (e) => {
            ...
            document.documentElement.setAttribute('data-bs-theme', newTheme); // Update Bootstrap theme
        });
    }
    ...
    // Filter options
    document.querySelectorAll('#timeRemainingOptions .btn').forEach(btn => {
        btn.addEventListener('click', () => {
            tempFilters.timeRemaining = btn.dataset.value === 'all' ? 'all' : parseInt(btn.dataset.value);
            // Add/remove active class for Bootstrap buttons
            btn.parentNode.querySelectorAll('.btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
        });
    });
    document.querySelectorAll('#minVolumeOptions .btn').forEach(btn => {
        btn.addEventListener('click', () => {
            tempFilters.minVolume = parseInt(btn.dataset.value);
            // Add/remove active class for Bootstrap buttons
            btn.parentNode.querySelectorAll('.btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
        });
    });
    document.querySelectorAll('#minLiquidityOptions .btn').forEach(btn => {
        btn.addEventListener('click', () => {
            tempFilters.minLiquidity = parseInt(btn.dataset.value);
            // Add/remove active class for Bootstrap buttons
            btn.parentNode.querySelectorAll('.btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
        });
...
function renderFilterTags(searchQuery = '') {
    ...
    filteredTags.forEach(([tag, count]) => {
        ...
        chip.className = `btn btn-outline-primary btn-sm tag-chip${tempFilters.tags.includes(tag) ? ' active' : ''}`;
        chip.innerHTML = `${tag} <span class="badge bg-secondary ms-1">${count}</span>`;
...
function updateActiveFiltersDisplay() {
    ...
    if (!hasFilters) {
        container.innerHTML = '<span class="text-muted">Click the "Filters" button to add filters</span>';
    }
...
function renderWeekView(searchQuery = '') {
    ...
    weekDates.forEach(dateKey => {
        ...
        dayEl.className = `col-md week-day${isToday ? ' today' : ''}`;
        ...
        dayEl.innerHTML = `
            <div class="week-day-header">
                ...
                ${dayEvents.length > 0 ? `<div class="badge bg-secondary">${dayEvents.length} events</div>` : ''}
            </div>
        ...
        if (dayEvents.length === 0) {
            eventsContainer.innerHTML = '<div class="text-center text-muted p-3 small">No events</div>';
        } else {
...
function showDayEvents(dateKey) {
    ...
    document.getElementById('eventModalLabel').textContent = `${dateStr} 만료 예정`;
    ...
    dayEvents.forEach(event => {
        ...
        const eventEl = document.createElement('div');
        eventEl.className = 'list-group-item list-group-item-action';
        eventEl.onclick = () => openEventLink(event.slug, event._searchQuery);
        eventEl.innerHTML = `
            <div class="d-flex w-100 justify-content-between">
                <h5 class="mb-1">${event.title}</h5>
                <span class="badge bg-${probClass} rounded-pill">${prob}%</span>
            </div>
            <p class="mb-1">${event.category || 'Uncategorized'}${marketCount > 1 ? ` · ${marketCount} markets` : ''}</p>
        `;
